...
class company_jobs_listTag {
    ...
    public function run(){
        ...
        if($this->params['page']){
            ...
            $company_info = $model->where($this->where)->order('company_id')->limit($this->firstRow . ',' .$this->spage_size)->group('company_id')->getfield('distinct company_id,count(id) as jobs_num');
            $result['page'] = '';
            $result['total'] = 0;
        }
        if($company_info){
