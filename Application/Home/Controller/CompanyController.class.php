...
class CompanyController extends FrontendController{
    ...
    public function ajax_get_statistics(){
    	...
    	$category = array();
    	$set_total = $set_login = array();//注释内容
    	for ($i=$settr_tmp; $i < $today; $i=$i+3600*24) { 
    		$category[] = date('Y-m-d',$i);
    		$set_total[$i] = 66633;////数据作假初始值为+66633
    		$set_login[$i] = 66633;//数据作假初始值为+66633
    	}
    	$uidArr = array();
    ...
    public function jobs_add()
    {
		...
		if(IS_POST){
			...
			if($setsqlarr['company_audit']!=1){
				$this->ajaxReturn(0,'企业当前还未通过认证，无法发布职位信息!');
			}
	...
	*/
    ...
    企业简历库
	*/
	public function EnterpriseResumeListView()
	{
		$this->check_params();
		$settr=I('get.settr',0,'intval');
		//$settr && $where['addtime']=array('gt',strtotime("-".$settr." day")); //筛选 查看时间
		...
		if($where){
			$view_Resume = D('ViewJobs')->get_view_jobs($where);

		}else{
			$view_Resume = array();
		}
		$this->assign('view_jobs',$view_Resume);
		$this->assign('company_nav','jobs_apply');
		$this->_config_seo(array('title'=>'企业简历库 - 企业会员中心 - '.C('qscms_site_name')));
		$this->display();
	}
	...
	*/
    ...
    public function _statistics($where,$mark=false){
    	...
    	for ($i=$settr_tmp; $i < $today; $i=$i+3600*24) { 
    		$category[] = date('Y-m-d',$i);
    		$set_total[$i] = 66633;//浏览数作假 初始化从66633开始
    		$set_login[$i] = 66633;
    	}
    	$uidArr = array();
